    ...
    calculate = function(self, card, context)
        if context.setting_blind and G.GAME.blind.boss then
            if card.ability.extra.hands_remaining > 0 then
                card_eval_status_text(context.blueprint_card or card, 'extra', nil, nil, nil,
                    { message = "+" .. tostring(card.ability.extra.hands_remaining) .. " Hand", colour = G.C.GREEN })
        ...
        if context.end_of_round and context.game_over == false and context.main_eval then
            if G.GAME.current_round.hands_left > 1 then
                card.ability.extra.hands_remaining = card.ability.extra.hands_remaining + G.GAME.current_round
                    .hands_left
            end
